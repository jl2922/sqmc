module more_tools
  ...
  public ::  signmod, constrained_dets, fermionic_phase, get_nbr,check_nbr, is_in_list, is_in_list_mod,                        &
           ...
           & cyrus_diagonalize_sparse, davidson_sparse, davidson_sparse_single
...
!=====================================================================================================================
 ...
 real(rk)                 :: epsilon=1.e-10 ! for Lanczos
...
!real(rk)                 :: rannyu
 ...
 integer :: n_diagonalize
  ...
  write(6,'(/,''Executing davidson_sparse in more_tools.f90'')')
  ...
  if (n>1) then
    ...
    do it=n_states+1,niter
       ...
       if (hamiltonian_type.eq.'hubbardk' .or. hamiltonian_type.eq.'chem' .or. hamiltonian_type.eq.'heg') then
         ...
         call fast_sparse_matrix_multiply(n,matrix_indices,nelem_nonzero,matrix_values,v(:,it),Hv(:,it))
       endif
 ...
 subroutine davidson_sparse_single(n,lowest_eigenvector,lowest_eigenvalue,matrix_indices,nelem_nonzero,matrix_values,highest_eigenvalue,initial_vector)
 ! Diagonally pre-conditioned Davidson
 ! A Holmes, 17 Nov 2016
 ...
 integer :: ind
  ...
  if (n>1) then
    ...
    ! First iteration:
      ...
      call fast_sparse_matrix_multiply(n,matrix_indices,nelem_nonzero,matrix_values,v(:,1),Hv(:,1))
    endif
    lowest_eigenvalue = dot_product(v(:,1),Hv(:,1))
    lowest_eigenvalue_prev = lowest_eigenvalue
    write(6,'(''Iteration, Eigenvalue='',i3,f16.9)') 1, lowest_eigenvalue
    w(:) = v(:,1)
    Hw(:) = Hv(:,1)
    ...
    do it=2,iterations
       ! Compute residual
       v(:,it) = (Hw(:) - lowest_eigenvalue*w(:))/(lowest_eigenvalue - diag_elems(:))
       if (abs(lowest_eigenvalue-diag_elems(1))<1e-8_rk)  v(1,it) = -1._rk  ! Since denominator could be 0
      ...
      !call print_real_matrix(it,it,h_krylov(1:it,1:it))
       ...
       if (present(highest_eigenvalue))  highest_eigenvalue=max(highest_eigenvalue,eigenvalues(it))
       if (it.gt.1 .and. abs(lowest_eigenvalue-lowest_eigenvalue_prev)<epsilon) then
           converged=.true.
           exit
       else
    ...
    it=min(it,iterations)
    write(6,'(''davidson_sparse: iter, n, Lowest eigenvalue ='',i4,i10,f17.10)') it, n, lowest_eigenvalue ; call flush(6)
  ...
  deallocate (v,Hv,w,Hw,h_krylov,h_overwrite,eigenvalues,diag_elems,work)
...
!=====================================================================================================================
  subroutine sparse_matrix_multiply(matrix_indices,matrix_values,vector,answer)
!=====================================================================================================================
  ...
  do i=1,size(matrix_values)
    answer(matrix_indices(i,1)) = answer(matrix_indices(i,1)) + matrix_values(i) * vector(matrix_indices(i,2))
  enddo
...
!=====================================================================================================================
  subroutine fast_sparse_matrix_multiply(n,matrix_indices,nelem_nonzero,matrix_values,vector,answer)
!=====================================================================================================================
  ! Multiplies a sparse matrix by a (non-sparse) vector
  ! Returns a vector that is assumed to be the same size as the input vector
...
!=====================================================================================================================
  subroutine from_upper_triangular_to_band(n,my_n,my_columns,matrix_indices,nelem_nonzero,matrix_values)!,my_rows)
!=====================================================================================================================
...
!=====================================================================================================================
    ...
    do while (lower_bound <= upper_bound)
      ...
      else if (search_for_up == temp_det_up .and. search_for_dn == temp_det_dn) then
        return
...
! End of edit
 ...
 do i=1,16
    ...
    else if (new_dets_up(i) .eq. rep_up) then
