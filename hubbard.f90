...
!           call flush(6)
    ...
    else if (hamiltonian_type .eq. 'hubbarddm') then                                                     ! DM basis calculations
...
!       if (i==1) wfsign=lowest_eigenvec(i)/wf_val
        ...
        else if(lowest_eigenvec(i)*wf_val*wfsign.le.-1.d-16) then
...
!  spr=.true.
 ...
 if ((dn_diff_1 .eq. 0) .and. (dn_diff_2 .eq. 0)) then
     if ((up_diff_1 .eq. -1) .and. (up_diff_2 .eq. 1)) then
        up=.true.
        electron_lost=.true.
     else if ((up_diff_1 .eq. 1) .and. (up_diff_2 .eq. -1)) then
        up=.true.
        electron_lost=.false.
     else
        matrix_element=0._rk
 ...
 else if ((up_diff_1 .eq. 0) .and. (up_diff_2 .eq. 0)) then
     if ((dn_diff_1 .eq. -1) .and. (dn_diff_2 .eq. 1)) then
        up=.false.
        electron_lost=.true.
     else if ((dn_diff_1 .eq. 1) .and. (dn_diff_2 .eq. -1)) then
        up=.false.
        electron_lost=.false.
     else
        matrix_element=0._rk
...
!============================================================================================================================
  ...
  !----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    do i = 1, n_det
       ...
       else if (hamiltonian_type .eq. 'hubbardk') then
   ...
   !if (ipr) then
   ! write(6,'(/,''Wavefunction has'',i8,'' determinants:'',/,'' det_up   det_dn,  cdet'')')  n_det
...
!   write(6,'(/,''Wavefunction has'',i8,'' determinants. Dets. with largest'',i3,'' abs coefs are:'',/,'' det_up   det_dn    cdet'')')  n_det,min(n_det,20)
          ...
          if (marked(j) .eqv. .false.) then
              if (chosen .eqv. .false.) then
                ...
                if (nup_dm(j) .lt. nup_dm_present) then
                    ...
                    chosen_j=j
                else if (nup_dm(j) .eq. nup_dm_present .and. ndn_dm(j) .lt. ndn_dm_present) then
                    nup_dm_present=nup_dm(j)
                    ndn_dm_present=ndn_dm(j)
                    dm_eig_present=dm_eigenvalues(j)
                    chosen_j=j
                else if (nup_dm(j) .eq. nup_dm_present .and. ndn_dm(j) .eq. ndn_dm_present .and. dm_eig_present .lt. dm_eigenvalues(j)) then
                    nup_dm_present=nup_dm(j)
                    ndn_dm_present=ndn_dm(j)
                    dm_eig_present=dm_eigenvalues(j)
                    chosen_j=j
  ...
  subroutine twiddle_determinant(det_spin)
    ...
    ! ---------------------------------------------------------------------------------------------------------------------
        ...
        do excite_level=0,max_excite_level
          if(sum_w2_excit_level(excite_level).ne.0_rk) write(6,'(2i8,f12.6)') excite_level, popul_excit_level(excite_level), sum_w2_excit_level(excite_level)
        enddo
