...
public :: mpi_allred, mpi_agathv, mpi_gathv, mpi_red, mpi_stop
...
type t_walk
    real(rk)     :: wt
    integer(i8b) :: det_u(2*num_words),det_d(2*num_words)
...
subroutine init_mpitype_walker
  ...
  integer :: block_len(0:3)
  integer :: oldtypes(0:3),offsets(0:3)
  ...
  oldtypes(1) = MPI_INTEGER8
  oldtypes(2) = MPI_INTEGER8
  oldtypes(3) = MPI_INTEGER1
  ...
  call MPI_TYPE_STRUCT(4,block_len,offsets,oldtypes,MPI_WALK_TYPE,mpierr)
...
#ifdef MPI
  !free the window
  call MPI_WIN_FREE(shmemWindow,mpierr)
...
subroutine mpi_sendwalks(isnd_start,my_nwalk,my_cnt,my_nwalk_1,initiator)
...
!------------------------------------------------------------------------------
subroutine mpi_push_nwalk(iw,iinitiator)
...
subroutine mpi_sendnewwalks(my_nwalk,initiator)
    integer(i1b), intent(inout) :: initiator(:)
    integer,intent(inout) :: my_nwalk
...
subroutine shmem_allocate(array,numberOfElements)
 ...
 !if MPI is defined, use the MPI sharedmenmory things
  ...
  !want the sizeof
  shmem_buf_size = ceiling(real(numberOfElements/my_ncores_on_node))*sizeof(1._rk)
  !Allocate shared memory. Arguments: bufferSize, elementSize, MPI_INFO
  !communicator,c_ptr for array,window,mpierr
  call MPI_WIN_ALLOCATE_SHARED(shmem_buf_size,sizeof(1._rk),MPI_INFO_NULL,sharedComm,array_local_cptr,shmemWindow,mpierr)
  !query the master processor (of the window [so, of the sharedComm, or of the node]) for 
  !the location of the start of the memory.
  call MPI_WIN_SHARED_QUERY(shmemWindow,0,shmem_buf_size,shmem_buf_size,array_cptr,mpierr)
