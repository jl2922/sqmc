module do_walk
  ...
  use common_run, only: run_type, proposal_method, ipr, partial_node_eps,importance_sampling, connected_dets_up, connected_dets_dn,use_efficient_heatbath
...
! ------------------------------------------------------------------------------
  ...
  use chemistry, only : counter_two_body,hamiltonian_chem, time_sym, hamiltonian_chem_time_sym, find_connected_dets_chem,setup_orb_by_symm
  ...
  use common_selected_ci, only : eps_var, eps_pt, target_error, dump_wf_var, n_states
...
! read and set 2 random number seeds, the first for the Hamiltonian and the second for the walk
! so that we can do different walks for the same Hamiltonian.
  if(master_core) read(5,'(4i4,x,4i4)') irand_seed
  call mpi_bsend(irand_seed)
  irand_seed(:,2) = irand_seed(:,2) + whoami ! gives every core a different seed for run (but same seed for hamiltonian)
  write(6,'(/,''random number seeds [master_core]'',t25,4i4,x,4i4)') irand_seed
...
! initiator_rescale_power is used to rescale r_initiator during equilibration when both tau and r_initiator are made larger than their final values
  ...
  if(master_core) read(5,*) run_type ! run_type can be vmc, fixed_node1, fixed_node2, fixed_node3, partial_node, release_node, no_fixed_node, sr, SR, none, trunc_lanc, or selected_ci, but some have not been implemented yet.
  ...
  elseif (run_type.eq.'hci') then
    if (master_core)  read(5,*) eps_var, eps_pt, target_error, n_states
    if (master_core)  read(5,*) dump_wf_var
  endif
  ...
  ! Run type is MANDATORY. You must enter one of the below - HJC May 14,2011
  if((run_type .eq.'fixed_node1') .or. (run_type .eq. 'fixed_node2') .or. (run_type .eq. 'fixed_node3') .or. (run_type .eq. 'fixed_node4') .or. (run_type .eq. 'partial_node') .or. (run_type .eq. 'release_node') .or. (run_type .eq. 'vmc') .or. (run_type .eq. 'no_fixed_node') .or. (run_type .eq. 'none') .or. (run_type .eq. 'selected_ci') .or. (run_type .eq. 'trunc_lanc') .or. (run_type .eq. 'sr').or. (run_type .eq. 'SR').or.(run_type.eq.'cisdtq').or.(run_type.eq.'cisd_pt').or.(run_type.eq.'hci')) then
    write(6,'(''run_type= '',a)') trim(run_type)
  else
    write(6,'(''run_type= '',a)') run_type ; call flush(6)
  ...
  write(6,'(''energy_exact='',9f10.5)') energy_exact
  call flush(6)

! Set random number seed for the Hamiltonian
  call setrn(irand_seed(1,1))

 ! Read parameters specific to system, set up Hamiltonian matrix, possibly diagonalize it, find m_connected_dets (the maximum number of connected dets)
  call hamiltonian
...
!Warning: Is this in the right place for a parallel run?
    ...
    !if (print_stuff .and. iwalk==2)  stop "DEBUGGING"
...
!  if (my_nwalk<my_ndet_psi_t_connected(whoami+1)+my_ndet_outside_ct(whoami+1))  stop 'nwalk<ndet_psi_t_connected+ndet_outside_ct'
  if (nwalk<ndet_psi_t_connected+ndet_outside_ct)  stop 'nwalk<ndet_psi_t_connected+ndet_outside_ct'
!***e
  i_permanent_initiator=0
...
! ------------------------------------------------------------------------------
 ...
 !write (6,*) "merge_original_with_spawned3 called; nwalk=",nwalk,", ndet_psi_t_connected=",ndet_psi_t_connected,", ndet_outside_ct=",ndet_outside_ct
 !call flush(6)
  if (my_nwalk<my_ndet_psi_t_connected+my_ndet_outside_ct)  stop 'nwalk<ndet_psi_t_connected+ndet_outside_ct'
  i_permanent_initiator=0
  leave_out=0 ! Number of dets in outside_ct that will be discarded
  num_to_move=0 ! Number of newly spawned dets to insert into the list outside of ct
  merge_with_prev=.false. ! Whether a walker that is not in ct or outside_ct needs to be merged with the one to its left
