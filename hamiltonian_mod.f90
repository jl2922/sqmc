...
!***
  ...
  use mpi_routines, only : whoami
  use common_selected_ci, only : cdets, tdets, eps_var, eps_pt, target_error, n_states
...
!   write(6,'(''nelec,norb,ndet='',9i5)') nelec,norb,ndet
  ...
  elseif(hamiltonian_type.eq.'heg') then
    ...
    if (run_type .eq. 'hci') then
      ...
      call perform_hci(eps_var,eps_pt,target_error,n_states=n_states)
  ...
  elseif(hamiltonian_type.eq.'chem') then
    ...
    if (run_type.eq.'hci') then
      ...
      if (hf_up==0_ik) then
        ...
        call perform_hci(eps_var,eps_pt,target_error,n_states,hf_up,hf_dn)
      endif
    ...
    if (diff.eq.2) then
        ...
        if (pbc.eqv. .false.) then
            if ((x_1.eq. x_2) .and. abs(y_1-y_2) .eq. 1) then
                allowed=.true.
            else if ((y_1.eq. y_2) .and. abs(x_1-x_2) .eq. 2) then
                allowed=.true.
            else
                allowed=.false.
            endif
            ...
            if ((x_1.eq. x_2) .and. (signmod(y_1-y_2,l_y) .eq. 1) ) then
                allowed=.true.
            else if ((y_1.eq. y_2) .and. (signmod(x_1-x_2,2*l_x) .eq. 2)) then
                allowed=.true.
            else
                allowed=.false.
            endif
...
subroutine read_psit
  ...
  if (master_core) then
    ...
    if (print_psit_wo_sqmc) then
      ...
      write (6,*) "Performing sqmc run","whoami",whoami; call flush(6)
    endif
