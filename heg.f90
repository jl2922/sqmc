...
!  private
  ...
  type(rs_absH),allocatable, target :: dtm_hb(:,:)
...
contains
  ...
  subroutine read_heg
    ...
    implicit none
    real(rk) energy, kinetic_energy, exchange_energy
#ifdef NUM_ORBITALS_GT_127
...
!   if(nup.eq.5 .and. ndn.eq.5) call test_off_diagonal_move_heg()
    ...
    write(6,'(/,''For this cell, HF kinetic, exchange, total energies ='',9f14.8)') kinetic_energy, exchange_energy, kinetic_energy+exchange_energy
    call theoretical_hf_heg()
    if(n_dim.eq.3) call madelung_energy(energy)
  ...
  subroutine setup_efficient_heatbath_heg()
    ...
    type(rs_absH), pointer :: dtm_hb_ptr

    write(6, *) 'setup_efficient_heatbath_heg called.'
    write(6, *) 'run_type:', run_type
    write(6, *) 'hamiltonian_type:', hamiltonian_type

    if (proposal_method.ne.'fast_heatbath')  return

    if (run_type.eq.'hci') then
      call my_second(1, 'allocate dtm_hb')
      allocate(dtm_hb(combine_2_indices(norb,2*norb),norb)) ! assumes p<q, since combine_2_indices is invariant under interchange of p and q
          ...
          do r = 1, norb
            do s = r, norb
              ...
              if (H_ijkl .ne. 0._rk) then
                ...
                dtm_hb_ptr => dtm_hb(occ_pair_ind, unocc_pair_ind)
                dtm_hb_ptr%r = r
                dtm_hb_ptr%s = s
                dtm_hb_ptr%absH = H_ijkl
              end if
            end do
          end do
          call sort_rs_absH(dtm_hb(occ_pair_ind, 1:unocc_pair_ind))
          max_double = max(max_double, dtm_hb(occ_pair_ind, 1)%absH)
        end do
      end do
      ...
      do p = 1, norb
        ...
        do q = norb + p, 2 * norb
          ...
          do r = 1, norb
            do s = 1, norb
              H_ijkl = abs(double_excitation_matrix_element_no_ref(p, q, r, s + norb))
              if (H_ijkl .ne. 0._rk) then
                unocc_pair_ind = unocc_pair_ind + 1
                dtm_hb_ptr => dtm_hb(occ_pair_ind, unocc_pair_ind)
                dtm_hb_ptr%r = r
  ...
  subroutine generate_k_vectors()
    ...
    !local variables
    integer :: i, j, k, index, n_max
    real(rk), allocatable :: values(:), temp(:, :)
    real(rk), parameter :: epsilon = 1.e-6_rk
    character(len=3) :: fmt

    !k points can only take integer values (in units of 2 pi / L) in each coordinate
    ...
    write(fmt, '(i1)') n_dim
    do i = 1, norb
       write(6,'(' // trim(fmt) // 'f10.4)')  k_vectors(:, i)
    enddo
   call flush(6)
  ...
  subroutine hamiltonian_heg(det_i_up, det_i_dn, det_j_up, det_j_dn, matrix_element)
    ...
    real(rk), parameter :: EPS = 1.e-10_rk
    real(rk) :: FOUR_PI
    ...
    if (det_i_up == det_j_up .and. det_i_dn == det_j_dn) then
      ...
      do i = 1, n_eor_up
        ...
        if (btest(det_i_up, orb_id - 1)) then
          ...
          if (.not. momentum_q_set) then
            orb_q = orb_id
            momentum_q_set = .true.
          else if (.not. momentum_s_set) then
            orb_s = orb_id
            momentum_s_set = .true.
          end if
        end if
      ...
      if (sum(momentum_change**2) * length_cell**2 > EPS) then
        return
      end if
...
!     close(2)
    ...
    write(6,'(/,''Wavefunction has'',i6,'' determinants. Dets. with largest'',i3,'' abs coefs are:'',/,'' det_up   det_dn,  cdet'')')  n_det,min(n_det,20)
...
! Calculate what tau_optimal_deterministic and tau_optimal_stochastic would be if the wavefn were this trial wavefn.
  ...
  subroutine find_important_connected_dets_heg(det_up, det_dn, min_elem, n_connected_dets, connected_dets_up, connected_dets_dn, matrix_elements, ref_diag_elem, diag_elems_info)
  ...
  ! A Holmes, 22 Feb 2016
    ...
    do ipair=1,npairs
      ...
      if (p > norb .and. q > norb) then
        p2 = p - norb
        q2 = q - norb
      else if (p <= norb .and. q > norb .and. p > q - norb) then
        p2 = q - norb
        q2 = p + norb
      end if
      ...
      do hpair=1,max_h_pair
        if (dtm_hb(epair,hpair)%absH<=min_elem)  exit
  ...
  subroutine find_connected_dets_heg(det_up, det_dn, n_connected_dets, connected_dets_up, connected_dets_dn, matrix_elements)
    ...
    real(rk), intent(out), allocatable,optional :: matrix_elements(:)
#ifdef NUM_ORBITALS_GT_127
    type(ik_vec), intent(in) :: det_up
    type(ik_vec), intent(in) :: det_dn
    type(ik_vec), intent(out) :: connected_dets_up(:), connected_dets_dn(:)
    ...
    call find_important_connected_dets_heg( & 
        & det_up, det_dn, 1.e-10_rk, n_connected_dets, connected_dets_up, connected_dets_dn, matrix_elements)
  ...
  !===========================================================================
  subroutine madelung_energy(energy)
    !---------------------------------------------------------------------------
    ...
    if (n_dim /= 3) then
       stop 'Madelung energy is only implemented for 3d'
    endif
    ...
    do i = 1, size(g_vectors, 2)
       ...
       energy = energy + exp(- sum(g_vectors(:, i)**2)/(2*kappa)**2) / sum(g_vectors(:, i)**2)
    enddo
    energy = energy * 4 * pi / length_cell**3
    energy = energy - pi / length_cell**3 / kappa**2 - 2 * kappa / pi ** 0.5_rk
    energy = energy * nelec / 2._rk

    write(6,'(''Madelung energy ='', f10.6)') energy
  ...
  subroutine theoretical_hf_heg()
    ...
    write (6,'(/,''Theoretical Hartree-Fock Results'')')
    write (6,'(a,f6.4)') "r_s = ", r_s
    write (6,'(a,i1)') "n_dim = ", n_dim
    ...
    write (6,'(a)') "For the HF determinant"
    write (6,'(a,f10.6)') "Kinetic energy per particle = ", kinetic_energy
    write (6,'(a,f10.6)') "Exchange energy per particle = ", exchange_energy
    write (6,'(a,f10.6)') "Total energy per particle = ", kinetic_energy + exchange_energy
    write (6,'(a,i3)') "Scaled by number of electrons = ", nelec
    write (6,'(a,f14.6)') "Kinetic energy = ", kinetic_energy * nelec
    write (6,'(a,f14.6)') "Exchange energy = ", exchange_energy * nelec
    write (6,'(a,f14.6)') "Total energy = ", (kinetic_energy + exchange_energy) * nelec
...
!      call off_diagonal_move_heg(det_i_up, det_i_dn, weight_i, det_j_up, det_j_dn, weight_j, diagonal_matrix_element)
  ...
  ! A Holmes, 11 Mar 2016
    ...
    if (p<=norb.and.q<=norb) then ! same spin, up
      call hamiltonian_heg(ibset(ibset(zero_det,p-1),q-1),zero_det,ibset(ibset(zero_det,r-1),s-1),zero_det,elem)
    elseif (p>norb.and.q>norb) then ! same spin, dn
      call hamiltonian_heg(zero_det,ibset(ibset(zero_det,p-norb-1),q-norb-1),zero_det,ibset(ibset(zero_det,r-norb-1),s-norb-1),elem)
    else ! opposite spin
      call hamiltonian_heg(ibset(zero_det,p-1),ibset(zero_det,q-norb-1),ibset(zero_det,r-1),ibset(zero_det,s-norb-1),elem)
    endif
